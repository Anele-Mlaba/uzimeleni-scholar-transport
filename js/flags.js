// ============================================================
// FLAGS / COMPLIANCE — Zimeleni Scholar Transport System
// ============================================================
...
async function renderFlags() {
  ...
  if (hasRole('owner')) {
    _renderOwnerComplianceView();
  } else {
    _renderAdminFlagsView();
  }
}

function _renderOwnerComplianceView() {
  const user        = getCurrentUser();
  const flaggedList = getFlags();
  const myFlag      = flaggedList.find(f => f.owner.id === user.ownerId);

  const unpaidAmt = getPayments()
    .filter(p => p.ownerId === user.ownerId && p.status === 'pending')
    .reduce((s, p) => s + p.amount, 0);
  const absentCount = getMeetings().reduce((count, m) => {
    const owner = getOwnerById(user.ownerId);
    return count + ((m.absentees || []).some(a => owner && a.idNumber === owner.idNumber) ? 1 : 0);
  }, 0);

  document.getElementById('flags-content').innerHTML = `
    <div class="mb-4">
      <h5 class="mb-0 fw-semibold">My Compliance Status</h5>
    </div>
    ...
    <div class="alert alert-info border-0 shadow-sm mb-4 d-flex gap-3">
      ...
      <div>
        ...
        <span class="small">You will be flagged if you:</span>
    ...
    ${myFlag
           ...
           <div class="card-body">
             <div class="d-flex gap-3 align-items-start">
               <div class="rounded-circle bg-danger-subtle text-danger d-flex align-items-center
                           justify-content-center flex-shrink-0" style="width:48px;height:48px;font-size:1.2rem">
                 <i class="bi bi-exclamation-triangle-fill"></i>
               </div>
               <div class="flex-grow-1">
                 <div class="fw-semibold fs-6 text-danger mb-1">You are currently flagged</div>
                 <div class="mt-2">
                   ${myFlag.reasons.map(r => `
                 ...
                 <div class="mt-3 d-flex gap-4 text-muted small">
                   ${absentCount > 0 ? `<span><i class="bi bi-calendar-x me-1"></i>${absentCount} meeting(s) missed</span>` : ''}
                   ${unpaidAmt > 0 ? `<span><i class="bi bi-cash me-1"></i>${formatCurrency(unpaidAmt)} outstanding</span>` : ''}
                 </div>
               </div>
               <span class="badge bg-danger px-3 py-2 flex-shrink-0">Flagged</span>
             </div>
           </div>
         </div>`
      : `<div class="card border-0 shadow-sm">
           <div class="card-body text-center py-5">
             ...
             <h5 class="mt-3 text-success">You're in good standing</h5>
             <p class="text-muted mb-0">No compliance issues on your account.</p>
             <div class="mt-3 d-flex justify-content-center gap-4 text-muted small">
               <span><i class="bi bi-calendar-check me-1 text-success"></i>${absentCount} meeting(s) missed</span>
               <span><i class="bi bi-cash me-1 text-success"></i>${formatCurrency(unpaidAmt)} outstanding</span>
             </div>
           </div>
         </div>`
    }
...
function _renderAdminFlagsView() {
  const flaggedList = getFlags();
                ...
                <div class="d-flex flex-column flex-md-row justify-content-between gap-3">
                  ...
                  <div class="d-flex flex-md-column gap-2 align-items-start align-items-md-end">
                    ...
                    <div class="text-end d-none d-md-block">
                      ${absentCount >= 3 ? `<div class="small text-muted">${absentCount}/${getMeetings().length} meetings missed</div>` : ''}
